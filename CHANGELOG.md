...
### 重构
- 重写 `main.py` 主入口，彻底移除重复引擎类，统一通过 `engines/manager.py` 调度四个引擎
- 统一 HTTP `/analyze` 与 `/choose-move` 的请求校验、响应格式、非法走法校验与随机合法走法兜底
- 统一运行时引擎选项解析、别名规范化、配置保存与重启后读回逻辑

### 修复
- 修复 `random_seed` 固定时每个局面都返回同一步的问题，现在会结合 FEN、depth 与合法走法数量生成可复现但随局面变化的随机结果
